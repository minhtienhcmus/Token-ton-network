...
let minterContract:OpenedContract<Xoffer>;
...
const mintAction = async (provider:NetworkProvider, ui:UIProvider) => {
    ...
    const nanoMint     = BigInt(mintAmount);
    // const curState     = await (provider.api() as TonClient).getContractState(minterContract.address);
...
export async function run(provider: NetworkProvider) {
    ...
    const minterCode = await compile('Xoffer');
    let   done   = false;
    let   retry:boolean;
    ...
    minterContract = provider.open(Xoffer.createFromAddress(minterAddress));
